	...
	switch(action.type) {
		...
		case 'USERS_USER': {
			if (action.responseJson !=  null && action.responseJson.params != null) {
				...
				for (let i = 0; i < appForms.ADMIN_USER_FORM.length; i++) {
					let classModel = JSON.parse(appForms.ADMIN_USER_FORM[i].classModel);
					if (action.responseJson.params.item != null && action.responseJson.params.item[classModel.field]) {
						inputFields[appForms.ADMIN_USER_FORM[i].name] = action.responseJson.params.item[classModel.field];
					} else {
						let result = "";
						if (appForms.ADMIN_USER_FORM[i].value != null && appForms.ADMIN_USER_FORM[i].value != ""){
							let formValue = JSON.parse(appForms.ADMIN_USER_FORM[i].value);
						...
						inputFields[appForms.ADMIN_USER_FORM[i].name] = result;
					}
				}
